...
namespace Gremlin.Net.Structure.IO.GraphBinary.Types
{
    ...
    public class PSerializer : SimpleTypeSerializer<P>
    {
        ...
        protected override async Task<P> ReadValueAsync(Stream stream, GraphBinaryReader reader)
        {
            ...
            {
                return new P(operatorName, SafelyConvertToP(args[0]), SafelyConvertToP(args[1]));
            }

            if (operatorName == "not")
            {
                return new P(operatorName, SafelyConvertToP(args[0]));
            }
        ...
        private static P SafelyConvertToP([NotNull] object? pObject)
        {
