...
public class GroovyTranslatorTests
{
    ...
    [Fact(Skip = "WithOptions not supported yet")]
    public void ShouldTranslateOptions()
    {
        AssertTranslation("WithOptions.tokens", WithOptions.Tokens);
    }
        ...
        var expectedGroovyScriptByTraversal = new Dictionary<ITraversal, string>
        {
            ...
            },
            {
                _g.WithStrategies(new VertexProgramStrategy()).V().ShortestPath()
                    .With("~tinkerpop.shortestPath.target", __.Has("name", "peter")),
